#!/usr/bin/env bash
# Smoke test for scripts/rollback-92730034-s1-94978278.sh. Run as root on a
# DISPOSABLE Linux box (VM or container) - it creates users, writes /var/lib
# and /etc/systemd, and kills processes. Never run on a real validator.
#
#   sudo bash scripts/rollback-92730034-s1-94978278-smoke.sh [manual|systemd|pre-reboot|post-reboot|all]
#
...
SRC="$REPO_DIR/scripts/rollback-92730034-s1-94978278.sh"
...
WWW="$T/www"
WORK=/var/lib/harmony-recovery-92730034-s1-94978278
BIN="$WORK/bin/harmony-recovery"
PRIV="$WORK/private"
STATE="$PRIV/state"
SENT_DIR=/run/harmony-recovery-92730034-s1-94978278
...
SAFE_CHILD_HASH="0x1111111111111111111111111111111111111111111111111111111111111111"
...
rpc_set() { # rpc_set [bn] [target-hash] [keys-json] [shard-id] [child-hash]
  local bn="${1-94978350}" h="${2-$TARGET_HASH}" keys="${3-[\"$K2\",\"$K1\"]}"
  local shard_id="${4-1}" child="${5-$SAFE_CHILD_HASH}"
  printf '{"bn": %s, "blockhash": "%s", "childhash": "%s", "blskey": %s, "shard-id": %s, "network": "mainnet", "role": "Validator"}\n' \
    "$bn" "$h" "$child" "$keys" "$shard_id" > "$T/rpc.json"
}

rpc_set_port() { # <port> [keys-json]
  printf '{"bn": 94978350, "blockhash": "%s", "childhash": "%s", "blskey": %s, "shard-id": 1, "network": "mainnet", "role": "Validator"}\n' \
    "$TARGET_HASH" "$SAFE_CHILD_HASH" "${2-[\"$K3\"]}" > "$T/rpc-$1.json"
}
...
class Rpc(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if m == 'hmyv2_getNodeMetadata':
            ...
            height = (body.get('params') or [None])[0]
            result = {'hash': c['childhash'] if height == 94978279 else c['blockhash']}
        else:
            result = None
...
new_manual_case() { # <name>; sets INV, DATA
  ...
  DATA="$INV/data"
  rm -rf "$INV"
  mkdir -p "$DATA/harmony_db_1"
  printf 'x' > "$DATA/harmony_db_1/CURRENT"
  printf 'old' > "$DATA/harmony_db_1/olddata"
  make_companion_db "$DATA"
...
m_happy() {
  ...
  expect "m_happy/prepare" 0 "^READY [0-9a-f]{96}(,[0-9a-f]{96})* recovery-92730034-s1-94978278$" || return 0
  ...
  [[ "$RB_OUT" == "READY $BLS_SORTED recovery-92730034-s1-94978278" ]] \
  ...
  diff -r "$T/dbsrc" "$DATA/harmony_db_1" >/dev/null 2>&1 \
    && ok "m_happy/new-installed" || bad "m_happy/new-installed" "installed tree differs from source"
  [[ "$(stat -c %U "$DATA/harmony_db_1")" == "hmytest" ]] \
    && ok "m_happy/new-owned" || bad "m_happy/new-owned" "$(stat -c %U "$DATA/harmony_db_1")"
  [[ "$(cat "$DATA/harmony_db_0/companion-marker")" == "must-survive" ]] \
    && ok "m_happy/companion-preserved" || bad "m_happy/companion-preserved"
  ...
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_happy/prepare-rerun-idempotent" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
...
m_source_mismatch() {
  ...
  [[ -z "$(ls -A "$DATA/.hmy-recovery-92730034-s1-94978278/db" 2>/dev/null)" ]] \
    && ok "m_srcmetric/nothing-transferred" || bad "m_srcmetric/nothing-transferred"
  # The frozen profile is part of persistent state. Simulate replacing the
  # rejected, pre-destructive test artifact with a newly frozen script.
  rm -rf "$WORK" "$DATA/.hmy-recovery-92730034-s1-94978278"
  mk_script
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_srcmetric/rerun-ready" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
}
...
m_resume_partial() {
  ...
  local dst="$DATA/.hmy-recovery-92730034-s1-94978278/db/harmony_db_1" ino
  mkdir -p "$dst"
  cp -p "$T/dbsrc/000001.ldb" "$dst/000001.ldb"
  chown -R hmytest: "$DATA/.hmy-recovery-92730034-s1-94978278"
  ino="$(stat -c %i "$dst/000001.ldb")"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_resume/ready" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  [[ "$(stat -c %i "$DATA/harmony_db_1/000001.ldb")" == "$ino" ]] \
...
m_bad_source() {
  ...
  [[ -d "$DATA/harmony_db_1" && -f "$DATA/harmony_db_1/olddata" ]] \
...
m_tampered_installed() {
  ...
  mv "$DATA/harmony_db_1/LOG" "$T/saved-LOG"
  ln -s /etc/passwd "$DATA/harmony_db_1/LOG"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  ...
  rm -f "$DATA/harmony_db_1/LOG"
  mv "$T/saved-LOG" "$DATA/harmony_db_1/LOG"
  chown hmytest: "$DATA/harmony_db_1/LOG"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_tamperdb/recovers" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}

m_wrong_head() {
  # The head pin is enforced after start over RPC (the raw-directory transfer
  # has no content hash): a wrong hash for block 94,978,278 quarantines the
  ...
  rpc_set 94978350 "0xbadbadbadbadbadbadbadbadbadbadbadbadbadbadbadbadbadbadbadbadbad0"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  ...
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_wronghead/recovers-after-clear" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}

m_wrong_shard() {
  # A shard-0 RPC accidentally selected with an otherwise valid validator
  # layout must be refused before download, stop, or database replacement.
  cleanup_case; new_manual_case wrongshard; mk_script; start_orig
  rpc_set 94978350 "$TARGET_HASH" "[\"$K2\",\"$K1\"]" 0
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_wrongshard/refused" 1 "^STOPPED unsupported-layout [0-9-]+$"
  orig_running && ok "m_wrongshard/node-untouched" || bad "m_wrongshard/node-untouched"
  [[ -f "$DATA/harmony_db_1/olddata" && -f "$DATA/harmony_db_0/companion-marker" ]] \
    && ok "m_wrongshard/dbs-untouched" || bad "m_wrongshard/dbs-untouched"
  rpc_set
}

m_rejected_child() {
  # A DB exposing the exact abandoned shard-1 child is quarantined and
  # latched even though the retained target hash is correct.
  cleanup_case; new_manual_case rejectedchild; mk_script; start_orig
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_rejectedchild/ready" 0 "^READY " || return 0
  rpc_set 94978350 "$TARGET_HASH" "[\"$K2\",\"$K1\"]" 1 "$REJECTED_CHILD_HASH"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_rejectedchild/latched" 1 "^STOPPED head-mismatch [0-9-]+$"
  [[ -z "$(pids_by_exe "$BIN")" ]] \
    && ok "m_rejectedchild/node-stopped" || bad "m_rejectedchild/node-stopped"
  [[ "$(state_get HEAD_MISMATCH)" == *"rejected shard-1 child"* ]] \
    && ok "m_rejectedchild/reason-recorded" || bad "m_rejectedchild/reason-recorded"
  rpc_set
}
...
m_latch_active() {
  ...
  setsid bash -c "exec 3< '$DATA/harmony_db_1/CURRENT'; sleep 300" >/dev/null 2>&1 < /dev/null &
  local holder=$!
  sleep 1
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
...
m_low_disk() {
  ...
  [[ ! -e "$DATA/.hmy-recovery-92730034-s1-94978278/db" ]] \
...
m_discard() {
  ...
  expect "m_discard/ready" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  local old; old="$(state_get OLD_DB_NAME)"
  ...
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_discard/resume-deleting" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
  ...
  sed -i -e 's/^STATE=.*/STATE=DELETING/' -e 's/^OLD_DB_DISPOSITION=.*/OLD_DB_DISPOSITION=kept/' "$STATE"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_discard/absent-after-deleting" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
  [[ "$(state_get OLD_DB_DISPOSITION)" == "deleted" ]] \
...
m_duplicate() {
  ...
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
  expect "m_dup/ready" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
...
m_rootless() {
  ...
  expect "m_rootless/prepare" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  ...
  expect "m_rootless/start" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  ...
  RB_SERVICE="$ABSENT_UNIT" run_rb_user hmytest "$INV" start
  expect "m_rootless/start-rerun" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
  kill_fakes
}
...
write_state_manual() { # <state> <old-present:cur-present:staged-present>
  local st="$1" spec="$2"
  local old="harmony_db_1.pre-recovery-test"
  install -d -m 0711 "$WORK"; install -d -m 0755 "$WORK/bin"; install -d -m 0700 "$PRIV"
  cp -f "$T/fake-recovery" "$BIN"; chmod 0755 "$BIN"
  rm -rf "${DATA:?}/harmony_db_1" "${DATA:?}/$old" "${DATA:?}/.hmy-recovery-92730034-s1-94978278"
  ...
  if [[ "$curp" == 1 ]]; then
    ...
    cp -a "$T/dbsrc" "$DATA/harmony_db_1"
  fi
  if [[ "$stagedp" == 1 ]]; then
    mkdir -p "$DATA/.hmy-recovery-92730034-s1-94978278/db"
    cp -a "$T/dbsrc" "$DATA/.hmy-recovery-92730034-s1-94978278/db/harmony_db_1"
  fi
  cat > "$STATE" <<EOF
BLS_IDS=$BLS_SORTED
CONFIG=$INV/harmony.conf
DATADIR=$DATA
DB_BYTES=$(db_src_bytes "$T/dbsrc")
DB_FILE_COUNT=$(db_src_count "$T/dbsrc")
...
REJECTED_CHILD_HASH=$REJECTED_CHILD_HASH
REJECTED_CHILD_HEIGHT=94978279
RUN_CWD=$INV
RUN_USER=hmytest
RECOVERY_SUFFIX=recovery-92730034-s1-94978278
STATE=$st
...
m_state_matrix() {
  ...
  for variant in "SWAP_BEGUN:0:1:1" "SWAP_BEGUN:1:0:1" "OLD_RENAMED:1:0:1" \
    ...
    RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
    expect "m_matrix/$st-$spec" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || continue
    [[ "$(state_get STATE)" == "READY" && -f "$DATA/harmony_db_1/MANIFEST-000001" && -d "$DATA/harmony_db_1.pre-recovery-test" ]] \
...
m_bad_oldname() {
  ...
  sed -i 's|^OLD_DB_NAME=.*|OLD_DB_NAME=harmony_db_1.pre-recovery-test/../../victim-outside|' "$STATE"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
...
m_start_reconcile() {
  ...
  sed -i 's/^STATE=.*/STATE=STARTING/' "$STATE"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_reconcile/starting-adopt" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
  ...
  sleep 1
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_reconcile/started-down-restart" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"

  # STARTED but unhealthy (wrong blskey set): stopped again, then recovers.
  rpc_set 94978350 "$TARGET_HASH" "[\"$K1\"]"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
...
m_failed_start() {
  ...
  rm -f "$DIE"
  RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
  expect "m_failstart/recovers" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}
...
new_systemd_case() {
  ...
  mkdir -p /home/harmony/data/harmony_db_1 /home/harmony/blskeys /home/harmony/logs /etc/harmony
  : > /home/harmony/p2p.key
  printf 'x' > /home/harmony/data/harmony_db_1/CURRENT
  printf 'old' > /home/harmony/data/harmony_db_1/olddata
  make_companion_db /home/harmony/data
...
s_multiple_units() {
  ...
  mkdir -p /home/rbsmoke-b/data/harmony_db_1
  printf 'x' > /home/rbsmoke-b/data/harmony_db_1/CURRENT
  printf 'old' > /home/rbsmoke-b/data/harmony_db_1/olddata
  make_companion_db /home/rbsmoke-b/data
...
EOF
  ...
  expect "s_multi/a-prepare" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  ...
  expect "s_multi/a-start" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  run_rb /root prepare --systemd-unit "$UNIT_B"
  expect "s_multi/b-prepare" 0 "^READY $K3 recovery-92730034-s1-94978278$" || return 0
...
s_happy() {
  ...
  run_rb /root prepare
  expect "s_happy/prepare" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$" || return 0
  ...
  [[ -f /etc/systemd/system/harmony.service.d/50-harmony-recovery-s1-94978278-exec.conf ]] \
    && ok "s_happy/exec-dropin-present" || bad "s_happy/exec-dropin-present"
  grep -q -- '--consensus.aggregate-sig=false' /etc/systemd/system/harmony.service.d/50-harmony-recovery-s1-94978278-exec.conf \
  ...
  [[ "$(stat -c %U "$SDATA/harmony_db_1")" == "harmony" ]] \
    && ok "s_happy/new-owned" || bad "s_happy/new-owned"
  [[ "$(cat "$SDATA/harmony_db_0/companion-marker")" == "must-survive" ]] \
    && ok "s_happy/companion-preserved" || bad "s_happy/companion-preserved"

  run_rb /root prepare
  expect "s_happy/prepare-rerun" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
  ...
  run_rb /root start
  expect "s_happy/start-rerun" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}

s_predeleted_db() {
  # Supervised low-space case: systemd is stopped and the old DB was already
  # deleted. This ambiguous first run requires SHARD1 even with --quiet.
  new_systemd_case; mk_script
  systemctl stop harmony.service
  rm -rf /home/harmony/data/harmony_db_1
  sed -i 's/--http.port=9500/--http.port=19500/' /etc/systemd/system/harmony.service
  systemctl daemon-reload
  run_rb /root prepare --discard-old-db --quiet
  expect "s_predeleted/confirmation-required" 1 "^STOPPED shard-confirmation-cancelled [0-9-]+$"
  [[ ! -e /home/harmony/data/harmony_db_1 && -f /home/harmony/data/harmony_db_0/companion-marker ]] \
    && ok "s_predeleted/no-change-before-confirmation" || bad "s_predeleted/no-change-before-confirmation"
  run_rb_input SHARD0 /root prepare --discard-old-db
  expect "s_predeleted/wrong-confirmation-refused" 1 "^STOPPED shard-confirmation-cancelled [0-9-]+$"
  [[ ! -e /home/harmony/data/harmony_db_1 ]] \
    && ok "s_predeleted/wrong-confirmation-no-change" || bad "s_predeleted/wrong-confirmation-no-change"
  run_rb_input SHARD1 /root prepare --discard-old-db
  expect "s_predeleted/ready" 0 "^READY unknown recovery-92730034-s1-94978278$" || return 0
  [[ "$(systemctl is-active harmony.service)" != "active" ]] \
    && ok "s_predeleted/unit-stopped" || bad "s_predeleted/unit-stopped"
  [[ -f /home/harmony/data/harmony_db_1/MANIFEST-000001 ]] \
...
s_leftover_hold() {
  ...
  cat > /etc/systemd/system/harmony.service.d/99-harmony-recovery-s1-94978278-hold.conf <<EOF
[Unit]
...
EOF
  ...
  systemctl daemon-reload
  run_rb /root start
  expect "s_leftover/heals" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
  [[ ! -f /etc/systemd/system/harmony.service.d/99-harmony-recovery-s1-94978278-hold.conf ]] \
...
s_unhealthy_post() {
  ...
  rpc_set
  run_rb /root start
  expect "s_unhealthy/recovers" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}
...
s_latch_active() {
  ...
  rm -f /etc/systemd/system/harmony.service.d/99-harmony-recovery-s1-94978278-hold.conf
  systemctl daemon-reload
  systemctl start harmony.service >/dev/null 2>&1 || true
  wait_for 10 bash -c '[[ "$(systemctl is-active harmony.service)" == "active" ]]' \
...
s_failed_start() {
  ...
  rm -f "$DIE"
  run_rb /root start
  expect "s_failstart/recovers" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}
...
s_started_restart_failure() {
  ...
  rm -f "$DIE"
  run_rb /root start
  expect "s_restartfail/recovers" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
}
...
EOF
  chmod 755 "$T/fakectl/systemctl"
  rpc_set 94978350 "0x2222222222222222222222222222222222222222222222222222222222222222"
  set +e
  RB_OUT="$(cd /root && PATH="$T/fakectl:$PATH" bash "$RB" start)"
  RB_RC=$?
  set -e
...
s_needs_root() {
  ...
  rm -rf /home/hmytest/.hmy-recovery-92730034-s1-94978278
}
...
s_cgroup_refusal() {
  ...
  rm -rf "$inv"; mkdir -p "$inv/data/harmony_db_1"
  printf 'x' > "$inv/data/harmony_db_1/CURRENT"
  make_companion_db "$inv/data"
  cp "$T/fake-orig" "$inv/harmony"
  cat > "$inv/harmony.conf" <<EOF
Version = "2.6.2"
...
pre_reboot() {
  ...
  if have_systemd; then
    new_systemd_case; mk_script
    run_rb /root prepare
    expect "pre-reboot/systemd-ready" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
  else
    new_manual_case reboot; mk_script; start_orig
    RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" prepare
    expect "pre-reboot/manual-ready" 0 "^READY $BLS_SORTED recovery-92730034-s1-94978278$"
  fi
...
post_reboot() {
  if have_systemd; then
    ...
    run_rb /root start
    expect "post-reboot/systemd-start" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
  else
    ...
    RB_SERVICE="$ABSENT_UNIT" run_rb "$INV" start
    expect "post-reboot/manual-start" 0 "^RUNNING $BLS_SORTED recovery-92730034-s1-94978278$"
  fi
}
